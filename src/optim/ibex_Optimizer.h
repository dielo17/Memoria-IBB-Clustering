...
struct ClusteringParams {
    enum class Algorithm {  
		KMEANS_PLUS,
		HDBSCAN 
	}; // Nuevo enum para elegir

    Algorithm choice = Algorithm::KMEANS_PLUS;   // Algoritmo por defecto

	// HDBSCAN
    int hdbscan_min_cluster_size = 5;     // Tamaño mínimo de clúster
    int hdbscan_min_samples = -1;         // -1 = usar min_cluster_size
...
struct RestartStats {
	...
	int successful_restarts = 0;    // Reinicios que redujeron buffer o mejoraron loup
    int failed_restarts = 0;        // Reinicios sin reducción ni mejora
...
private:
	...
	size_t buffer_size_before_restart = 0;      // Tamaño del buffer antes del reinicio
    double loup_before_restart = POS_INFINITY;  // Valor de loup antes del reinicio
